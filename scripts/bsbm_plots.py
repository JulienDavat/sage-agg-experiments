...
if input_file is None or output_directory is None:
    print('Error: missing required arguments ! USAGE: bsbm_plots.py --input <data> --output <output>')
    exit(1)
...
def plot_metric(ax, data, metric, title, xlabel, ylabel, logscale=False, display_x=True):
    df = data[['approach', 'dataset', metric]].groupby(['approach', 'dataset']).sum()
    dashes = [(2,2)] * data['approach'].nunique()
    chart = lineplot(x='dataset', y=metric, hue='approach', data=df, ax=ax, markers=True, style="approach", dashes=dashes)
    if logscale:
