...
for dir in args.dir:
    ...
    with open(dir + '/virtuoso.csv', 'r') as f:
    ...
    with open(dir + '/sage-opt-average.csv', 'r') as f:
        rows = csv.reader(f, delimiter=',')
        total_http_requests = 0
        total_traffic = 0
        total_execution_time = 0
        for row in rows:
            ...
            total_execution_time += float(row[0]) * 1000
            total_traffic += float(row[2])
        sageagg.append([total_http_requests, total_execution_time, total_traffic])
...
plt.plot(datasets, list(map(lambda e: e[0], sageagg)), label="sage-agg")
...
plt.plot(datasets, list(map(lambda e: e[1], sageagg)), label="sage")
...
plt.plot(datasets, list(map(lambda e: e[2], sageagg)), label="sage-agg")
plt.title('Traffic')
#plt.yscale('log')
#plt.show()
plt.legend()
